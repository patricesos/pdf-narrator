...
from PyQt6.QtWidgets import (
    QApplication,
    QFileDialog,
)  # Keep for file dialogs if preferred
from ttkbootstrap.constants import *

# Keep these imports - assuming they exist and work
from core.extract import extract_book
from core.generate_audiobook_kokoro import (
...
class SourceFrame(tb.Frame):
    ...
    def __init__(self, master, app, **kwargs):
        ...
        self.single_browse_btn = tb.Button(
            source_lf, text="Browse...", command=self._browse_single_pdf, width=10
        )
        ...
        self.batch_browse_btn = tb.Button(
            source_lf, text="Browse...", command=self._browse_pdf_folder, width=10
        )
        ...
        self.skip_entry = tb.Entry(
            source_lf, textvariable=self.manual_extracted_dir, width=50
        )
        self.skip_entry.grid(row=2, column=1, sticky="ew", padx=5, pady=5)
        self.skip_browse_btn = tb.Button(
            source_lf, text="Browse...", command=self._browse_extracted_folder, width=10
        )
        self.skip_browse_btn.grid(row=2, column=2, sticky="e", padx=(5, 0), pady=5)

        # --- Extraction Options (Only enabled if not 'skip') ---
        self.options_lf = tb.Labelframe(
            self, text="Extraction Options", padding=15, bootstyle=INFO
        )
        self.options_lf.grid(
            row=1, column=0, columnspan=3, sticky="ew", padx=10, pady=10
        )

        self.toc_check = tb.Checkbutton(
            self.options_lf,
            text="Use TOC/Metadata (if available)",
            variable=self.use_toc,
        )
        ...
        self.chapters_rb = tb.Radiobutton(
            mode_frame,
        ...
        self.whole_rb = tb.Radiobutton(
            mode_frame,
            text="Whole Book (Single File)",
            variable=self.extract_mode,
            value="whole",
        )
        ...
        out_lf = tb.Labelframe(
            self,
            text="Extracted Text Output (Preview)",
            padding=15,
            bootstyle=SECONDARY,
        )
        out_lf.grid(row=2, column=0, columnspan=3, sticky="ew", padx=10, pady=(10, 0))
        out_lf.grid_columnconfigure(0, weight=1)
        self.extracted_entry = tb.Entry(
            out_lf, textvariable=self.extracted_text_dir, state="readonly"
        )
        self.extracted_entry.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self.open_extracted_btn = tb.Button(
            out_lf, text="Open", command=self._open_extracted_folder, width=8
        )
    ...
    def _update_output_paths(self, source_path=None, source_type=None):
        ...
        print(
            f"  Initial Args: source_path='{source_path}', source_type='{source_type}'"
        )
        ...
        if not path:  # If called without specific path, get from current selection
            print("  Path not provided, attempting to get from state...")
            if option == "single":
                path = self.pdf_path.get()
            elif option == "batch":
                path = self.pdf_folder.get()
            elif option == "skip":
                path = self.manual_extracted_dir.get()
        ...
        if path_exists:
            ...
            if option == "single" and is_file:
                ...
                print(
                    f"  Warning: Path type doesn't match mode '{option}'. IsFile={is_file}, IsDir={is_dir}"
                )
        else:
            print("  Path is invalid or does not exist, cannot determine base_name.")
        ...
        if base_name:
            ...
            print("  Base_name is empty, output directories will be empty.")

        # Set the extracted text preview
        extracted_display_path = (
            extracted_dir if option != "skip" else path if path_exists else ""
        )
    ...
    def _browse_file_or_folder(self, mode):
        ...
        if mode == "single":
            ...
            if path:
                self.manual_extracted_dir.set(path)

        if path:
            self._update_output_paths(
                path, mode
            )  # Update paths immediately after selection
    ...
    def set_config(self, config):
        ...
        loaded_path = ""
        if loaded_option == "single":
            loaded_path = self.pdf_path.get()
        elif loaded_option == "batch":
            loaded_path = self.pdf_folder.get()
        elif loaded_option == "skip":
            loaded_path = self.manual_extracted_dir.get()
...
class AudioFrame(tb.Frame):
    ...
    def __init__(self, master, app, **kwargs):
        ...
        self.audio_format_display = tk.StringVar(
            value=".wav (High Quality)"
        )  # For combobox
        ...
        # --- Voice Selection ---
        voice_lf = tb.Labelframe(
            self, text="Voice Selection (Kokoro)", padding=15, bootstyle=INFO
        )
        voice_lf.grid(row=0, column=0, columnspan=2, sticky="ew", padx=10, pady=(0, 10))
        voice_lf.grid_columnconfigure(1, weight=1)

        tb.Label(voice_lf, text="Voice:").grid(
        ...
        # Audio Format
        tb.Label(settings_lf, text="Output Format:").grid(
            row=1, column=0, sticky="w", padx=(0, 10), pady=5
        )
        formats = [".wav (High Quality)", ".mp3 (Smaller Size)"]
        self.format_combo = tb.Combobox(
            settings_lf,
            textvariable=self.audio_format_display,
            values=formats,
            state="readonly",
            width=18,
        )
        ...
        # Device Selection
        tb.Label(settings_lf, text="Device:").grid(
            row=2, column=0, sticky="w", padx=(0, 10), pady=5
        )
        device_frame = tb.Frame(settings_lf)
        device_frame.grid(row=2, column=1, sticky="w", pady=5)
        self.gpu_rb = tb.Radiobutton(
            device_frame, text="GPU (CUDA)", variable=self.device, value="cuda"
        )
        self.gpu_rb.pack(side=LEFT, padx=(0, 15))
        self.cpu_rb = tb.Radiobutton(
        ...
        # --- Output Directory Display ---
        out_lf = tb.Labelframe(
            self, text="Audiobook Output (Preview)", padding=15, bootstyle=SECONDARY
        )
        out_lf.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=(10, 0))
        out_lf.grid_columnconfigure(0, weight=1)
        self.output_entry = tb.Entry(
            out_lf, textvariable=self.audio_output_dir, state="readonly"
        )
        self.output_entry.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self.open_output_btn = tb.Button(
            out_lf, text="Open", command=self._open_audio_folder, width=8
        )
    ...
    def update_output_display(self, path):
        ...
        self.open_output_btn.config(
            state=NORMAL if path and os.path.isdir(path) else DISABLED
        )
    ...
    def get_config(self):
        ...
        self.audio_format_display.set(
            format_map.get(self.audio_format.get(), ".wav (High Quality)")
        )
...
class ControlFrame(tb.Frame):
    ...
    def __init__(self, master, app, **kwargs):
        ...
        self.current_action_text = tk.StringVar(
            value="-"
        )  # e.g., "Extracting:", "Generating Audio:"
        self.current_file_text = tk.StringVar(
            value=""
        )  # e.g., "Chapter 1.txt" or "book.pdf"
        ...
        self.pause_button = tb.Button(
            ...
            text="⏸ Pause",
            bootstyle=WARNING,
            command=self.app.pause_process,
            state=DISABLED,
            width=10,
        )
        ...
        self.resume_button = tb.Button(
            ...
            text="⏯ Resume",
            bootstyle=INFO,
            command=self.app.resume_process,
            state=DISABLED,
            width=10,
        )
        ...
        tb.Label(status_frame, text="Status:", width=7).grid(
            row=0, column=0, sticky="w", padx=(0, 5)
        )
        tb.Label(status_frame, textvariable=self.status_text, font="-weight bold").grid(
            row=0, column=1, sticky="w"
        )
        tb.Label(status_frame, textvariable=self.estimated_time_text).grid(
            row=0, column=2, sticky="e", padx=10
        )

        # Row 2: Current Action & File
        tb.Label(status_frame, textvariable=self.current_action_text, width=15).grid(
            row=1, column=0, sticky="w", padx=(0, 5)
        )
        tb.Label(status_frame, textvariable=self.current_file_text).grid(
            row=1, column=1, sticky="w"
        )
        tb.Label(status_frame, textvariable=self.progress_count_text).grid(
            row=1, column=2, sticky="e", padx=10
        )

    # --- Update Methods (called by app) ---
    def update_status(
        self, status=None, action=None, file=None, count_str=None, est_time_str=None
    ):
...
class ProgressFrame(tb.Frame):
    ...
    def __init__(self, master, app, **kwargs):
        ...
        # --- Progress Bars ---
        progress_lf = tb.Labelframe(
            self, text="Detailed Progress", padding=15, bootstyle=INFO
        )
        progress_lf.grid(row=0, column=0, sticky="ew", padx=10, pady=(0, 10))
        progress_lf.grid_columnconfigure(
            1, weight=1
        )  # Make bars expand inside labelframe

        # Extract Progress
        tb.Label(progress_lf, text="📄 Text Extraction:", width=18).grid(
            row=0, column=0, sticky="w", padx=(0, 10)
        )
        self.extract_bar = tb.Progressbar(
            progress_lf, variable=self.extract_progress, maximum=100, bootstyle=INFO
        )
        self.extract_bar.grid(row=0, column=1, sticky="ew", pady=5)
        tb.Label(progress_lf, textvariable=self.extract_percent, width=5).grid(
            row=0, column=2, sticky="e", padx=(10, 0)
        )

        # Audio Progress
        tb.Label(progress_lf, text="🔊 Audio Generation:", width=18).grid(
            row=1, column=0, sticky="w", padx=(0, 10)
        )
        self.audio_bar = tb.Progressbar(
            progress_lf, variable=self.audio_progress, maximum=100, bootstyle=SUCCESS
        )
        self.audio_bar.grid(row=1, column=1, sticky="ew", pady=5)
        tb.Label(progress_lf, textvariable=self.audio_percent, width=5).grid(
            row=1, column=2, sticky="e", padx=(10, 0)
        )

        # --- Logs ---
        log_lf = tb.Labelframe(
            self, text="Process Logs", padding=(15, 10), bootstyle=SECONDARY
        )
        ...
        self.log_text = scrolledtext.ScrolledText(
            log_lf, height=10, wrap=tk.WORD, bd=0, relief="flat"
        )
...
class VoiceTestFrame(tb.Frame):
    ...
    def __init__(self, master, app, **kwargs):
        ...
        self.pause_event = (
            threading.Event()
        )  # For potential future pause/resume in test
        self.pause_event.set()

        # Variables
        self.test_text = tk.StringVar(
            value="This is a sample text to test the selected voice."
        )
        self.test_mode = tk.StringVar(value="single")
        self.selected_voice = tk.StringVar()  # Set from available voices
        self.test_output_dir = tk.StringVar(
            value=os.path.join(PROJECT_DIR, "voice_tests")
        )
        ...
        self.voice_lf = tb.Labelframe(
            self, text="Voice Selection", padding=15, bootstyle=INFO
        )
        # self.voice_lf is gridded in _update_ui
        self.voice_lf.grid_columnconfigure(1, weight=1)
        tb.Label(self.voice_lf, text="Select Voice:").grid(
            row=0, column=0, sticky="w", padx=(0, 10)
        )
        self.voice_list = available_voices()
        self.voice_combo = tb.Combobox(
        ...
        tb.Entry(out_frame, textvariable=self.test_output_dir, state=READONLY).grid(
            row=0, column=0, sticky="ew", padx=(0, 10)
        )
        tb.Button(
            out_frame, text="Open", command=self._open_output_folder, width=8
        ...
        # --- Progress Bar ---
        progress_lf = tb.Labelframe(
            self, text="Test Progress", padding=(15, 10), bootstyle=SECONDARY
        )
        ...
        self.progressbar = tb.Progressbar(
            progress_lf, variable=self.progress_var, maximum=100
        )
        self.progressbar.grid(row=0, column=0, sticky="ew", pady=5)
        self.progress_label = tb.Label(
            progress_lf, textvariable=self.progress_label_text
        )
    ...
    def _set_button_state(self, running):
        ...
        for child in (
            self.master.master.winfo_children()
        ):  # Access mode radio buttons via parent notebook tab
            if isinstance(child, tb.Labelframe) and "Mode" in child.cget("text"):
                for rb in child.winfo_children():
                    if isinstance(rb, tb.Radiobutton):
    ...
    def _start_test(self):
        ...
        if not test_text:
            messagebox.showwarning(
                "Input Required", "Please enter sample text to test."
            )
            return
        ...
        self.test_thread = threading.Thread(
            target=self._run_test_thread, args=(test_text,), daemon=True
        )
    ...
    def _run_test_thread(self, test_text):
        ...
        try:
            ...
            if mode == "single":
                ...
                if not voice:
                    raise ValueError("No voice selected for single test.")
                ...
                test_single_voice_kokoro(
                    ...
                    progress_callback=lambda p, fname, idx, total: (
                        self._progress_callback(p, voice)
                ...
                if not voices_to_test:
                    raise ValueError("No voices available to test.")
                ...
                try:
                    ...
                    print(f"Warning: Could not clean up temp test file: {e}")

            # --- GUI updates after completion (run via app.after) ---
            if not self.cancellation_flag:
                self.app.after(
                    0, lambda: self._progress_callback(final_progress, "Completed")
                )  # Ensure 100%
                ...
                self.app.after(
                    0, lambda: self._progress_callback(0, "Cancelled")
                )  # Reset progress bar
            ...
            self.app.after(
                0, lambda: self._progress_callback(0, "Error")
            )  # Reset progress bar
        finally:
    ...
    def _ask_open_folder(self):
        if messagebox.askyesno(
            "Test Complete", "Voice test finished. Open the output folder?"
        ):
...
class AudiobookApp(tb.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        tb.Separator(self, orient="horizontal").grid(
        ...
        self.control_frame.set_button_states(
    ...
    def _reset_config(self):
        if messagebox.askyesno(
            "Reset Settings", "Reset all source and audio settings to defaults?"
        ):
    ...
    def open_folder(self, folder_path):
        ...
        if not folder_path or not os.path.isdir(folder_path):
            messagebox.showwarning(
                "Folder Not Found", f"The folder does not exist:\n{folder_path}"
            )
            return
        try:
    ...
    def start_process(self):
        ...
        try:
            ...
            if (
                source_opt == "skip"
                and not self.config_data["source"]["manual_extracted_dir"]
            ):
            ...
            if source_opt != "skip" and not self.config_data["extracted_text_output"]:
                raise ValueError(
                    "Source Setup: Could not determine extracted text output directory."
                )
            if not self.config_data["audio_output"]:
                raise ValueError(
                    "Source/Audio Setup: Could not determine audiobook output directory."
                )
        ...
        self.process_thread = threading.Thread(
            target=self._run_process_thread, daemon=True
        )
    ...
    def cancel_process(self):
        if self.is_running:
            ...
            )  # Keep cancel active, disable others
    ...
    def _update_gui_progress(
        ...
        if action is not None:
            update_args["action"] = action
        if file is not None:
            update_args["file"] = file
        if count_str is not None:
            update_args["count_str"] = count_str
        if est_time_str is not None:
            update_args["est_time_str"] = est_time_str

        self.after(
            0,
            lambda: (
                self.progress_frame.update_progress(extract_p, audio_p),
                self.control_frame.update_status(**update_args)
                if update_args
                else None,
            ),
        )
        ...
        try:
            ...
            if source_opt != "skip":
                ...
                if source_opt == "single":
                    ...
                    self._update_gui_progress(
                        action="Extracting:", file=filename, count_str="(1 of 1)"
                    )
                    ...
                    def single_extract_progress(p):
                        ...
                        self._update_gui_progress(
                            est_time_str=f"Est. Time: {self._format_time(est)}"
                        )
                    ...
                    if not book_files:
                        raise FileNotFoundError(
                            "No PDF/EPUB/TXT/HTML files found in batch folder."
                        )
                    ...
                    for i, book_path in enumerate(book_files, start=1):
                        if self.cancellation_flag:
                            raise InterruptedError("Batch extraction cancelled")
                        ...
                        rel_path = os.path.relpath(
                            os.path.dirname(book_path), source_folder
                        )
                        book_name_no_ext = os.path.splitext(filename)[0]

                        # Output folder for this specific book's text
                        current_extract_output = os.path.join(
                            extracted_base_output, rel_path, book_name_no_ext
                        )
                        # Corresponding audio output folder
                        current_audio_output = os.path.join(
                            audio_base_output, rel_path, book_name_no_ext
                        )
                        ...
                        def batch_extract_progress(p):
                            ...
                            elapsed = time.time() - start_time
                            est = (
                                (elapsed / (overall_p / 100.0)) - elapsed
                                if overall_p > 0
                                else 0
                            )
                            self._update_gui_progress(
                        ...
                        all_task_folders.append(
                            (current_extract_output, current_audio_output)
                        )
                ...
                if not os.path.isdir(manual_dir):
                    raise FileNotFoundError(
                        "Existing text folder not found or is invalid."
                    )
                ...
                for root, _, files in os.walk(manual_dir):
                    if any(f.lower().endswith(".txt") for f in files):
                        ...
                        all_task_folders.append(
                            (text_input_folder, audio_output_folder)
                        )
                        found_text = True

                if not found_text and any(
                    f.lower().endswith(".txt") for f in os.listdir(manual_dir)
                ):
                ...
                if not all_task_folders:
                    raise FileNotFoundError(
                        f"No .txt files found within the specified existing text folder: {manual_dir}"
                    )

                print(
                    f"Found {len(all_task_folders)} text folder(s) to process for audio generation."
                )

            # --- 2. Audio Generation Phase ---
            if self.cancellation_flag:
                raise InterruptedError("Cancelled before audio generation")
            ...
            chunk_size = audio_cfg[
                "chunk_size"
            ]  # Not directly used by kokoro func? Check generate_audiobooks_kokoro
            device = audio_cfg[
                "device"
            ]  # Not directly used by kokoro func? Check generate_audiobooks_kokoro
            ...
            for task_idx, (text_input_dir, audio_output_dir) in enumerate(
                all_task_folders, start=1
            ):
                if self.cancellation_flag:
                ...
                self._update_gui_progress(
                    ...
                    file=task_name,
                    count_str=f"({task_idx} of {total_tasks})",
                )
                print(
                    f"--- Generating audio for: {task_name} ({task_idx}/{total_tasks}) ---"
                )

                os.makedirs(audio_output_dir, exist_ok=True)

                def audio_progress_callback(
                    progress, current_file="", file_idx=0, files_total=0
                ):
                    if self.cancellation_flag:
                        raise InterruptedError("Audio generation cancelled")

                    overall_progress = (
                        (task_idx - 1 + progress / 100.0) / total_tasks
                    ) * 100
                    self._update_gui_progress(audio_p=overall_progress)

                    # Update current file within the task
                    file_count_str = (
                        f"({file_idx}/{files_total})" if files_total > 0 else ""
                    )
                    self._update_gui_progress(
                        file=f"{task_name} / {current_file}",
                        count_str=f"{file_count_str}",
                    )  # Append file name

                    # Time estimate for audio phase
                    elapsed = time.time() - task_start_time
                    est = (
                        (elapsed / (overall_progress / 100.0)) - elapsed
            ...
            success_msg = (
                f"Process completed successfully in {self._format_time(total_time)}."
            )
            print(success_msg)
            self._update_gui_progress(
                status="Completed",
                ...
                file="",
                count_str="",
                est_time_str=f"Total Time: {self._format_time(total_time)}",
            )
            ...
            self.after(
                0, lambda: messagebox.showerror("Configuration Error", error_msg)
            )
        except InterruptedError as e:  # User cancellation
            error_msg = f"Process Cancelled: {e}"
            print(error_msg)
            self._update_gui_progress(
                status="Cancelled", action="-", file="", count_str=""
            )
            self.after(
                0,
            ...
            self.after(
                0,
                lambda: self.control_frame.set_button_states(
                    running=False, paused=False
                ),
            )
    ...
    def on_close(self):
        ...
        if (
            self.voice_test_frame.test_thread
            and self.voice_test_frame.test_thread.is_alive()
        ):
